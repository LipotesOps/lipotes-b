...
class DictionarySerializer(Serializer):
    ...
    def serialize_list(self, thelist):
        return [
            b64encode(pickle.dumps(v, protocol=pickle.HIGHEST_PROTOCOL))
            for v in thelist
        ]
    ...
    def deserialize_cancel(self, wf_spec, s_state):
        spec = Cancel(
            wf_spec, s_state["name"], success=s_state.get("cancel_successfully", False)
        )
        self.deserialize_task_spec(wf_spec, s_state, spec=spec)
        return spec
    ...
    def deserialize_choose(self, wf_spec, s_state):
        spec = Choose(wf_spec, s_state["name"], s_state["context"], s_state["choice"])
        self.deserialize_task_spec(wf_spec, s_state, spec=spec)
        return spec
    ...
    def deserialize_multi_instance(self, wf_spec, s_state):
        spec = MultiInstance(
            wf_spec, s_state["name"], times=self.deserialize_arg(s_state["times"])
        )
        self.deserialize_task_spec(wf_spec, s_state, spec=spec)
        return spec
    ...
    def deserialize_task(self, workflow, s_state):
        ...
        # children
        task.children = [
            self.deserialize_task(workflow, c) for c in s_state["children"]
        ]
