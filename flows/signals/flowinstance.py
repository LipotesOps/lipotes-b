...
def post_start_flow_instance(instance, raw, **kwargs):
    ...
    task_instance = TaskInstance(
        ...
        flowable_process_instance_id=flowable_process_instance_id,
    )
...
def post_start_event(instance, raw, **kwargs):
    ...
    resp = FR.request(
        uri="/runtime/tasks/{}".format(instance.flowable_task_instance_id),
        method="post",
        data=action_data,
    )
    if resp.status_code != 200:
    ...
    post_flowable_task_action.send_robust(
        sender="flows.TaskInstance", instance=instance, raw=raw, created=True
    )
...
def end_event(flowable_process_instance_id):
    ...
    将当前工单标记为已完成。
    """
    query_obj = FlowInstance.objects.get(
        flowable_process_instance_id=flowable_process_instance_id
    )
...
def sync_next_flowable_task_instance(instance, raw, created, **kwargs):
    ...
    if len(flowable_tasks) == 0:
        ...
        process_instance_resp = FR.getAProcessinstance(
            flowable_process_instance_id=flowable_process_instance_id
        )
    ...
    for task in flowable_tasks:
        ...
        obj, created = TaskInstance.objects.get_or_create(
            flowable_task_instance_id=id,
            task_definition_key=taskDefinitionKey,
            flow_instance=flow_instance,
            name=name,
...
def sync_flowable_task_list(instance, raw, created, **kwargs):
    ...
    resp = FR.request(
        uri="/repository/process-definitions/{processDefinitionId}/model".format(
            processDefinitionId=flowable_process_definition_id
        ),
        method="get",
        data={},
    )
    ...
    for task in flowElements:
        ...
        obj, created = Task.objects.get_or_create(
            name=name, task_definition_key=task_definition_key, flow_bpmn=flow_bpmn
        )
        pass
