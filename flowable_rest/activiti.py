    ...
    for item in out:
        ...
        if task_item['istask'] != True and task_item['isend'] != True:
            ...
            if expression == 'pass==0':
                pre_pass_tmp = None
            result.extend(_get_task_out(task_item, element_dict, pre_expression=pre_pass_tmp))
        else:
...
def dfs_task_in_count(task_id, task_in_dict, task_in_count, flag):
    ...
    for key in task_in_dict.get(task_id, []):
        result = result + dfs_task_in_count(task_id=key, task_in_dict=task_in_dict, task_in_count=task_in_count, flag=flag) + 1
...
def _get_task_list(init_item, element_dict):
    ...
    for i in range(task_count):
        ...
        dfs_task_in_count(task_id=task_id, task_in_dict=task_in_dict, task_in_count=task_in_count, flag={})
        ...
        if 'outgoingFlows' in item:
            ...
            for flow in item['outgoingFlows']:
                ...
                if not conditionExpression:
                    ...
                    try:
                        expression = re.search("\$\{(.*)\}", conditionExpression).group(1)
                    except:
...
def deploy(bpmn_data, org):
    ...
    method = 'POST'
    res = request_to_activiti(action=action, method=method, bpmn_data=bpmn_data, org=org)
